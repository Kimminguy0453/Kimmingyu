...
namespace JEngine
{
	...
	void BitMap::TransDraw(int x, int y, int size_x, int size_y, int size, int Cut)
	{
		AdjustAnchorPoint(x, y);
		if (size > Cut || (size == 0 && Cut == 0))
			TransparentBlt(ResoucesManager::GetInstance()->GetBackDC(), x, y, size_x, size_y, m_hMemDC, 0, 0, m_size.cx, m_size.cy, RGB(255, 0, 255));
		else if(size <= Cut)
			TransparentBlt(ResoucesManager::GetInstance()->GetBackDC(), x, y, (int)(size_x / Cut * size), size_y, m_hMemDC, 0, 0, (int)(m_size.cx / Cut * size), m_size.cy, RGB(255, 0, 255));
	}
