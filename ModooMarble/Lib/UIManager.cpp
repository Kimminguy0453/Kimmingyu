...
namespace JEngine
{
	...
	UIManager::UIManager()
	{
		SetBkMode(JEngine::ResoucesManager::GetInstance()->GetBackDC(), TRANSPARENT);//혼합 모드 설정 
	}
	...
	void UIManager::AddLabel(string str, int x, int y, DWORD drawOption, std::function<string()> callback)
	{
		Label *pNew = new Label();
		pNew->Init(str, x, y, drawOption, callback);
		m_vecUI.push_back(pNew);
	}
